...
from sqlalchemy import Boolean, Column, Date, ForeignKey, Index, String, Table, Text
from sqlalchemy import Enum as SQLEnum
from sqlalchemy.orm import Mapped, mapped_column, relationship

from .base import Base, TimestampMixin, created_at_dt, str_100, str_255, uuid_pk
...
patient_disease = Table(
    ...
    Column(
        "disease_id",
        ForeignKey("chronic_disease.id", ondelete="CASCADE"),
        primary_key=True,
    ),
)
...
class Patient(Base, TimestampMixin):
    ...
    __table_args__ = (
        Index("idx_patient_document", "document_number"),
    ...
    medical_background: Mapped["MedicalBackground"] = relationship(
        back_populates="patient", uselist=False, cascade="all, delete-orphan"
    )
    ...
    chronic_diseases: Mapped[list["ChronicDisease"]] = relationship(
        secondary=patient_disease, back_populates="patients"
    )
...
class PatientAccount(Base):
    ...
    patient_id: Mapped[uuid.UUID] = mapped_column(
        ForeignKey("patient.id", ondelete="CASCADE"), unique=True, nullable=False
    )
