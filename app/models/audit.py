...
from .base import Base, created_at_dt, str_100, uuid_pk

VariantJSON = JSON().with_variant(JSONB, "postgresql")  # Use JSONB for PostgreSQL, fallback to JSON for other DBs
...
class AuditLog(Base):
    """Immutable audit log populated automatically by DB triggers to track
    all changes to critical tables (appointments, patients, staff)"""
    ...
    old_data: Mapped[dict | None] = mapped_column(VariantJSON)
    new_data: Mapped[dict | None] = mapped_column(VariantJSON)
