...
from app.schemas.appointments_schema import TriageCreate, TriageResponse, TriageUpdate
...
async def registrar_triage(
    ...
    Requirements:
        ...
        - RBAC Enforcement: Accessible by authenticated NURSE or DOCTOR accounts.
    """
    ...
    nuevo_triage = await create_triage(
        db=db, appointment_id=appointment_id, triage_data=payload, nurse_id=current_user.id
    )
    return nuevo_triage
...
async def obtener_triage(
    ...
    Requirements:
        - Path Parameter: appointment_id (UUID)
        - RBAC Enforcement: Accessible by NURSE or DOCTOR roles for clinical auditing.
    """
    triage = await get_triage_by_appointment(db, appointment_id)
    if not triage:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Triage record not found for this appointment"
        )
    return triage
